...
>
  <img
    ...
    alt="altText"
  />
  <span v-if="isOpen" class="closed-message">Closed</span>
  <div class="card-body">
    <h5 class="cards-title">{{ title }}</h5>
    <p class="cards-times">
      {{opensAtTime}} to {{closesAtTime}}
     
    </p>
    <p class="card-text">{{ address }}</p>
    <a :href="generateRestaurantLink(id, cityId)" v-if="!isOpen" class="btn btn-success"
      >View Restaurant</a
    >
  </div>
</div>
...
const currentISTTimeFormatted = this.formatTime(currentISTTime);
if (currentISTTimeFormatted >= this.opensAtTime && currentISTTimeFormatted <= this.closesAtTime)
this.isOpen=false;
else
this.isOpen=true;
}
...
.closed-message {
  ...
  top: 50%; /* Position the message at the center of the image vertically */
  left: 50%; /* Position the message at the center of the image horizontally */
  transform: translate(-50%, -50%); /* Center the message exactly over the image */
  background-color: rgba(255, 255, 255, 0.8); /* Semi-transparent white background */
  padding: 5px 10px; /* Padding for the message */
  border-radius: 5px; /* Rounded corners */
  font-size: 14px; /* Font size of the message */
