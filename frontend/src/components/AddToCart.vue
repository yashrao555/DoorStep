<template>
    <div class="cart-page-container">
      ...
      <p class="location">{{ restaurant.operating_hours_start }} - {{ restaurant.operating_hours_end }}</p>
      <p class="location">Rating: {{ restaurant.rating }}</p>
    </div>

        <div class="cart-items">
      <div v-for="cartItem in cartItems" :key="cartItem.foodItem.food_item_id" class="cart-item">
        <div class="item-details">
          <h3>{{ cartItem.foodItem.name }}</h3>
          <div class="quantity-box">
            <span class="quantity-buttons" @click="updateQuantity(cartItem, -1)">-</span>
  ...
  mounted() {
    ...
    updateQuantity(cartItem, change) {
      ...
      this.$store.dispatch('updateCartItemQuantity', { foodItemId: cartItem.foodItem.food_item_id, quantity: newQuantity });
    },
    },
  };
  </script>
